...
| **R8** | **Protections côté GitHub, à activer dès la création du remote** (Settings → Branches → Branch protection rules) : `main` et `develop` protégées — push direct interdit, force-push interdit, suppression interdite, MR obligatoire avec **≥ 1 approbation** et **CI verte**, « Require linear history » sur `develop` (cohérent avec la stratégie rebase R5). |
