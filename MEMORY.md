...
- **Étape** : **#1/#3/#4/#5/#6 mergés dans `develop`** (`b0c69b7` ; #6 mergé le 20/07/2026). **Bilan de maturité/conformité produit** (`docs/Bilan_Maturite_Conformite.md`, vérifié par sous-agent) → **plan de correction lancé**. `docs` (bilan + parcours) et **lot S1 (durcissement local)** rebasés sur `develop`, prêts pour MR. Ensuite le renommage `mir-webapp` → `units-webapp`.
- **Branches** : `develop` = #1..#6 (`b0c69b7`). `main` au bootstrap. `docs/dossier-projet` et `feature/durcissement-securite` (S1) rebasées sur `develop`, prêtes pour MR. Puis `refactor/rename-units-webapp`.
- **Prochaine action** : MR `docs` + MR `S1` ; puis renommage `units-webapp` en une passe atomique ; puis S2 (dossier sécurité), S3 = durcissement infra (#9), S4 = CI/CD sécurité (#10). **Action utilisateur** : activer les protections de branches GitHub (R8).
...
| S1 | 🔄 **En cours (20/07/2026)** — Enablers de sécurité locale : gitleaks en pre-commit (+ `.gitleaks.toml`), mir-webapp non-root (nginx unprivileged, 8080), verrou de dépendances (`npm ci` si `package-lock.json`). | `feature/durcissement-securite` | Bilan §sécurité |
...
- **Lot S1** sur `feature/durcissement-securite` (depuis `develop`, R3) :
  ...
  - **mir-webapp non-root** : image `nginxinc/nginx-unprivileged` (uid 101, écoute 8080) ; `nginx.conf` (`listen 8080`) et port compose (`8090:8080`) adaptés. Build reproductible : `npm ci` si `package-lock.json` présent, sinon `npm install`.
- **Reste (Mac)** : `brew install gitleaks` ; committer `demo/mir-webapp/package-lock.json` (généré) pour activer `npm ci` ; recette du build mir-webapp non-root ; puis MR S1, puis renommage `mir-webapp` → `units-webapp`.
