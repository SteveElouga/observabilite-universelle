...
- **Étape** : **Backlog initial TERMINÉ et intégralement mergé dans `develop`** (`4ed4d7f`) : socle, démos units-service/units-webapp, alerting RED/SLO, sondes externes, plan de correction du bilan (S1..S4 + R8), astreinte OneUptime (#7), profiling Pyroscope (#8, **recette validée le 21/07/2026**). Pont log→trace corrigé (PR #24 mergée). Branche de démo en cours : `feat/demo-calcul` (endpoint CPU pour un flame graph parlant).
...
- **Prochaine action** : pousser et merger `feat/demo-calcul` (endpoint `/demo/calcul`), puis rebuild/recreate `units-service` pour l'activer. Ensuite, évolutions « à venir » du document maître, hors périmètre : dashboards RED/USE as-code (provisioning Grafana), montée à l'échelle Kubernetes + Mimir (§7.2). Reste aussi le volet organisationnel du bilan (politiques, évaluation de risque), volontairement hors périmètre.
...
| 8 | ✅ **Fait (20/07/2026)** — Profiling continu units-service. `requirements.txt` : `pyroscope-io==1.1.0` + `pyroscope-otel==1.1.0`. `gunicorn.conf.py` (`post_fork`) : `pyroscope.configure(app_name, server_address, sample_rate)` par worker (après fork, py-spy) + `PyroscopeSpanProcessor` sur le tracer provider auto-instrumenté → attribut `pyroscope.profile.id` sur le span racine (lien trace→profil, tracesToProfiles déjà en datasource). Compose units-service : `PYROSCOPE_SERVER_ADDRESS`, `cap_add: SYS_PTRACE`, `depends_on: pyroscope`. **Recette validée (21/07/2026)** : le conteneur en service datait d'avant #8 (ni paquet `pyroscope-io` ni env), corrigé par rebuild + recreate ; profils CPU Python ensuite émis et lus dans Pyroscope. Endpoint de démo `/demo/calcul` (CPU) ajouté (`feat/demo-calcul`). | `feature/pyroscope-sdk` | §10.1, §10.5 |
...
### 2026-07-21 — Session Claude : recette profiling #8 validée + endpoint `/demo/calcul`
- **Recette du lot #8 (profiling Pyroscope) validée en direct.** Diagnostic : `{service_name="units-service"}` restait à 0 sample alors que Pyroscope recevait bien les profils Go de la plateforme. Cause : le conteneur `units-service` en service **datait d'avant #8** (`pip show pyroscope-io` absent ; pas de `PYROSCOPE_SERVER_ADDRESS`). Correctif : `docker compose --profile demo up -d --build --force-recreate units-service`. Ensuite, profils CPU **Python** émis et lus (frames `SyncWorker`/`WSGIHandler`/Django, et le thread `BatchSpanProcessor` de l'exporter OTel visible dans la flamme).
- **Ajout `/demo/calcul`** (`feat/demo-calcul`) : vue `calcul()` + helper `_calcul_intensif` (boucle CPU en Python pur, bornée par `?n=`, défaut 5 M) + route `demo/calcul` + doc README. But : produire un flame graph **spectaculaire** (une seule fonction domine), là où les autres endpoints ne calculent presque rien. Après merge : rebuild/recreate units-service pour activer l'endpoint.
- Rappel : le pont log→trace corrigé sur `fix/loki-trace-derived-field` (regex du champ dérivé Loki, PR #24) est validé et en attente de merge.
